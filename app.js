...
app.get("/tweets/:tweetId/", authenticationToken, async (request, response) => {
  ...
  const getTweetIdsQuery = `select tweet_id from where user_id in (${getFollowingIds});`;
...
app.get(
  ...
  async (request, response) => {
    ...
    if (getTweetIds.includes(parseInt(tweetId))) {
      ...
      const getLikedUsersNames = getLikedUserNamesArray.map((eachUser) => {
...
const covertUserNameReplyedDBObjectToResponseObject = (dbObject) => {
  return {
    replies: dbObject,
  };
};
...
app.get(
  ...
  async (request, response) => {
    ...
    if (getTweetIds.includes(parseInt(tweetId))) {
      ...
      response.send(
        covertUserNameReplyedDBObjectToResponseObject(getUsernameReplyTweets)
      );
    } else {
      response.status(401);
      response.send("Invalid Request");
